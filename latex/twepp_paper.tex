...
Furthermore, the RD53 chip on a Single Chip Card (SCC) is read out over e-links using an FC7 mezzanine card~\cite{ref:fc7}.
...
E-links of two gauges (34 and 36 AWG) and different lengths (from 0.35 to 2.0 meters) are used to readout the RD53 chip.
...
In the hardware setup (left), an FC7 mezzanine card is connected to an SCC through two commercial display port cables, an adapter board, and an e-link for control and readout communication of an RD53 chip.
...
Two e-links (1.4 meters, 36 AWG) are twisted together (left), with a victim e-link connected between an FC7 mezzanine card and SCC to readout from an RD53 chip, and an aggressor e-link connected to a KC705.
