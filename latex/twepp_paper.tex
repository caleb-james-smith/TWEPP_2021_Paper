...
Cross talk effects from channels within an e-link bundle (internal) and from multiple bundles (external) is studied.
The RD53 chip on a Single Chip Card (SCC) is read out over e-links using an FC7 mezzanine card~\cite{ref:fc7}.
...
The readout of the RD53A chip across e-links is tested using an FC7 mezzanine card, as shown in \fig~\ref{fig:tap0_vs_length}.
...
In the hardware setup (left), an FC7 mezzanine card is connected to an SCC through two commercial display port cables, an adapter board, and an e-link for control and readout communication of an RD53A chip.
...
Two e-links (1.4 meters, 36 AWG) are twisted together (left), with a victim e-link connected between an FC7 mezzanine card and SCC to readout from an RD53A chip, and an aggressor e-link connected to a KC705.
